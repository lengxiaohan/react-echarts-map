...
var Container = React.createClass({
	...
	_getDatas: function(nums, type, page) {
		...
		if (!session) {
			...
			$.GetAjax($.getCtx() + '/rest/businesslist/getBusinessPage', setData, 'GET', true, function(data, state) {
				if (state && data.rows) {
	...
	render: function() {
		return (
			<div className="content-cp">
	        	...
	        	<FooterComponent next = {this._nextPage} prev = {this._prevPage} isCurrent = {this.state.pageNum == this.totalPage ? 1 : this.state.pageNum == 1 ? 0 : 2 } currentPage={this.state.pageNum} totalPage={this.state.totalPage}/>
        	</div>
		)
	}
});
