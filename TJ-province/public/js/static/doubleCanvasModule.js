...
(function($) {
	...
	var doubleCircle = {
		...
		setFontShow: function() {
			...
			var NUMUSS = 20;
			var result = (attrs['max'][0].parentX - attrs['max'][0].parentR) * 0.2;
			// 大圆中文字线条填充
			$.each(maxAttr, function(i, o) {
				...
				if (i == 0) {
					...
					that.ctx.dashedLineTo(o.x - 10, o.y + NUMUSS, result, o.y + NUMUSS, 0.1, color);
					that.pushFontShow('max', data['max'][i].name, result, o.y, '#fff');
					that.pushFontShow('max', values, result, o.y + 15, '#fff');
				} else {
					that.ctx.dashedLineTo(o.x, o.y, o.x - 10, o.y - NUMUSS, 0.1, color);
					that.ctx.dashedLineTo(o.x - 10, o.y - NUMUSS, result, o.y - NUMUSS, 0.1, color);
					that.pushFontShow('max', data['max'][i].name, result, o.y - 40, '#fff');
					that.pushFontShow('max', values, result, o.y - 25, '#fff');
				}
			});
			...
			$.each(minAttr, function(i, o) {
				...
				if (i == 0) {
					...
					that.pushFontShow('min', data['min'][i].name, that.WIDTH - result, o.y - 40, '#fff');
					that.pushFontShow('min', values, that.WIDTH - result, o.y - 25, '#fff');
				} else {
					that.ctx.dashedLineTo(o.x, o.y, o.x + 10, o.y + NUMUSS, 0.1, color);
					that.ctx.dashedLineTo(o.x + 10, o.y + NUMUSS, that.WIDTH - result, o.y + NUMUSS, 0.1, color);
					that.pushFontShow('min', data['min'][i].name, that.WIDTH - result, o.y, '#fff');
					that.pushFontShow('min', values, that.WIDTH - result, o.y + 15, '#fff');
				}
			});
		},
