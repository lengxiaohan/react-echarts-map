...
/**
 * @param 区县销售热度分布主函数，获取左边列表数据
 */
$(function() {
	var map = new Map();
	$.ajax({
		url:$.getCtx()+'/rest/areaSale/getAreaScale',
		dataType:'json',
		type:'GET',
		data:{areaId:$.getUrlParam("areaId"),needAll:true},
		success:function(data){
 ...
 * @param 绘制地图
 */
function countrySale(data,map) {
	require(
			[ 'echarts', 'echarts/chart/map' // 动态加载map
			  ],
			  function(echarts) {
				var myChart = echarts.init(document
						.getElementById('anyuemap'), 'macarons');
				/*var placeListbiger = [
				                      {
				                    	  name : '成都',
