...
var containerObj = {
    ...
    start: function() {
        ...
        $('.page1.active').animate({
            background:'-webkit-linear-gradient(top right, #0683e3, #095499)',
            background:'linear-gradient(top right,#0683e3,#095499)'
        },300);
    ...
    resizeCallback: function(){
        if ( $(window).width()/$(window).height() > 0.8 ) {
            ...
            $('.page1 > .fontlist1,.page1 > .fontlist2,.page3 > div,.page8 > div,.page9 > div,.page10 > div,.page11 > div').css({
                zoom: 320 / $(window).width()
            })
        }else{
            $('body,html').css({
                width: $(window).width()
