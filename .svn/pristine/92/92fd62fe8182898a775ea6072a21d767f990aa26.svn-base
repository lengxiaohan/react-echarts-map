...
// 获取网商列表类型
let areaShortName = $.getUrlParam('shortName') || '';
let times = $.getUrlParam('times') || 1;
			...
			return (
				<header>
					...
					<div className="head-con">
						...
						<p>{areaShortName}&nbsp;{this._getName()}</p>
					</div>
...
var SectionComponent = React.createClass({
	...
	componentDidMount: function() {
		// 接收请求返回数据
		this.props.getDatas($.cookie('PAGE') || 1, areaShortName,1);
	},
	render: function() {
		// 渲染列表数据
				...
				<section className="small">
					<div id="tableBox" className="table-box">
						<ul className="goodsListHeader">
							<li className="contentBox-style5">网商名称</li>
							<li className="contentBox-style6">主营范围</li>
						</ul>
				...
				<section>
					<div id="tableBox" className="table-box">
						<ul className="goodsListHeader">
							...
							<li className="contentBox-style2">所属平台</li>
							<li className="contentBox-style3">主营范围</li>
							<li className="contentBox-style4">类型 </li>							
						</ul>
						<div className="goodsListContent">
							{this.dom}
						</div>
		...
		if ( times == 0.25 ) {
			return (
				<ul className="contentBox" onClick={this._getListDataClick}>
					<li className="contentBox-style5" >{this.listData.item ? this.listData.item.name : ''}</li>
					<li className="contentBox-style6">{this.listData.item ? this.listData.item.mainAreas : ''}</li>
				</ul>
			)
		} else if ( times == 1 ) {
			return (
				<ul className="contentBox" onClick={this._getListDataClick}>
					...
					<li className="contentBox-style2">{this.listData.item ? this.listData.item.platName : ''}</li>
					<li className="contentBox-style3">{this.listData.item ? this.listData.item.mainAreas : ''}</li>
					<li className="contentBox-style4">{this.listData.item ? this.listData.item.type : ''}</li>
				</ul>
			)
		}
...
var Container = React.createClass({
	...
	componentWillMount: function() {
		this.PAGEERS = times == 0.25 ? 8 : 15; //每页总个数
		this.FETCHNUM = 10; //每页总个数
	...
	_getDatas: function(nums, name, page) {
		const that = this;
		let session = sessionStorage.getItem('sessionPage' + name + nums); //判断当前页是否有用户的缓存数据
			...
			$.GetAjax($.getCtx() + '/rest/network/page', setData, 'GET', true, function(data, state) {
				if (state && data.code == 0) {
					...
					for (let i = 0; i < data.rows.length / PAGEERS; i++) {
						...
						// 缓存
						sessionStorage.setItem('sessionPage' + name + (page * FETCHNUM - FETCHNUM + 1 + i), JSON.stringify(that.arrayGroup[i + (page * FETCHNUM - FETCHNUM)]));
						...
						if (i == 0 && page == 1) {
							...
							setTimeout(function(){
								$('.onLoading').remove();
								$('.content-cp').css("opacity",1);
								times == 0.25 ? $('.small').css("opacity",1) : '';
								that.setState({
									status: true,
	...
	_nextPage: function(event) {
		...
		if (this.state.pageNum >= this.totalPage) { //超过总页数退出
			...
			if (this.state.pageNum == this.nextPagerNum * this.FETCHNUM - 5) { //判断是否发送请求数据
				...
				sessionStorage.getItem('sessionPage' + areaShortName + (this.nextPagerNum * this.FETCHNUM + 1)) ? '' : this._getDatas(this.nextPagerNum * this.FETCHNUM + 1, areaShortName, this.nextPagerNum + 1);
				this.nextPagerNum++;
			}

			// 当前页数数据
			this._getDatas(this.state.pageNum, areaShortName);
