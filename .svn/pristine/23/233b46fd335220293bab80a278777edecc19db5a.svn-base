...
/*var LogoutButton = React.createClass({
    ...
    render: function(){
        var index="bu"+toString(this.listData+1);
        return(
             <span className={index}>{this.listData ? this.listData+1 : ''}</span>
            )
    }
});
var LoginButton = React.createClass({
    render: function(){
        return(
            <span className="button">{this.listData ? this.listData+1 : ''}</span>
            )
    }
});*/
...
var GetMapComponent = React.createClass({
    ...
    componentDidUpdate: function() {
        ...
        var dataMax=rowDatas[0].scale;
        var dataMin=rowDatas[30].scale;
        var dataone=((dataMin+dataAverage)/2).toFixed(1);
        var datatwo=((dataMax+dataAverage)/2).toFixed(1);   
        // var dom = this.refs.JsMap.getDOMNode();//this.refs.myInput.getDOMNode() 直接获取到组件的 DOM 节点。
