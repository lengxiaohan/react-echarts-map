...
/*			$.GetAjax(ctx+'/rest/info/recentAreaInfoListNew',{areaId:areaId,time:that.getCurrenTime()},'GET',true,function(data){
			...
			function callpack(data){
				...
				if(data && data[0] && data[1]){
					...
					$.each(data[0].x,function(i,b){
						if(i<=numsGoods){
							onex.push(b);
						}
					});
					
					$.each(data[0].y,function(i,b){
						if(i<=numsGoods){
							oney.push(b);
						}
					});
						...
						itemStyle: {
							normal: {
								...
								areaStyle: {
									...
									color: function () {
										var zrColor = zrender.tool.color;
										return zrColor.getLinearGradient(0, 0, 0, $('#main').height(), [[0, 'rgba(255,244,179,.8)'], [0.8, 'rgba(255,244,179,.1)']]);
									}()
								}
							}
						}
...
/*			$.GetAjax(
			...
			$.GetAjax($.getCtx() + '/rest/info/recentAreaInfoReal', { areaId: areaId, time: that.getCurrenTime(),getParent :true }, 'GET', true, function (data) {
				callpack(data);
			});
